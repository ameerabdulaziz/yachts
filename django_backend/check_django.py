#!/usr/bin/env python3
"""
Check Django availability and create minimal working setup
"""
import sys
import os
...
def check_django():
    try:
        ...
        print("Django not installed - will create simple Python API server")
        return False
...
def create_simple_api_server():
    """Create a simple Python API server as fallback"""
...
'''
    ...
    print("Created simple_api_server.py as Django fallback")

def main():
    print("=== Nauttec Django Setup Check ===")
    
    django_available = check_django()
    
    if not django_available:
        print("\\n⚠️  Django not available in current environment")
        print("Creating simple Python API server as alternative...")
    ...
    if missing_count > 0:
        ...
        print("Creating simple Python API as fallback...")
        create_simple_api_server()
        return False
    
    print("\\n🎉 All Django dependencies available!")
    print("Ready to run Django backend")
    return True
